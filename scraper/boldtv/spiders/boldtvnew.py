...
class BoldtvnewSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        h2s = response.css('.header-column h4 ::text').extract()
        if h2s[0] == 'I dag':
        ...
        for list_game in list_games:
            ...
            for match in matches:
                ...
                items['home_team'] = match.css('.home span span ::text').extract_first().strip()
                items['away_team'] = match.css('.away span span ::text').extract_first().strip()
                ...
                items['league'] = match.css('.tournament span::text').extract_first().strip()
                items['league_url'] = match.css('.tournament span::text').extract_first().strip()
